...
import { clampInt, presetNamesIn, resolveEmailSettings, serializeAccountsYaml } from './config.js';
...
export function createEmailRuntime(ctx, config, createPool = settings => new EmailPool(settings)) {
    ...
    const settingsScope = legacy ? ctx.settings.register(SETTINGS_NAMESPACE, EmailSettingsSchema, {
        ...
        get: () => {
            ...
            try {
                ...
                if (account)
                    endpointDefaults = { imap: account.imap, smtp: account.smtp };
            }
    ...
    const getEffectiveSettings = () => {
        ...
        if (!legacy)
            return resolveEmailSettings({ ...config });
