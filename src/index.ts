...
const REPLY_MODES: EmailReplyMode[] = ['reply', 'reply-all', 'forward']
...
const REPLY_LABELS: Record<EmailReplyMode, string> = {
  reply: '回复',
  'reply-all': '回复全部',
  forward: '转发',
}

function renderReply(value: EmailReplyResult): TextBlock[] {
  const rejected = value.rejected.length > 0 ? '；被拒：' + value.rejected.join(', ') : ''
...
,
  ...
  ctx.tools.register({
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown) => renderReply(value as EmailReplyResult),
    },
    async execute(rawArgs: unknown) {
