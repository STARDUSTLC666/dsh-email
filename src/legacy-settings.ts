...
function missing(legacy: Record<string, any>, current: Record<string, any>): Record<string, unknown> {
...
export async function importLegacySettings(ctx: any, config: Record<string, any>, namespace: string, entryId: string, fields: readonly string[], home?: string): Promise<boolean> {
  if (typeof ctx.settings.register === 'function' || config.legacySettingsImported === true) return false
  // Old namespaces were global. Do not copy credentials into unrelated custom instances.
  if (ctx.fiber?.entry?.options.id !== entryId) return false
  ...
  for (const file of ['settings.yaml.imported', 'settings.yaml']) {
    try { document = parse(readFileSync(resolve(base, file), 'utf8')); break }
    catch (error: any) { if (error.code !== 'ENOENT') throw new Error('Cannot read legacy settings; original file preserved') }
  }
  if (!plain(document) || !plain(document[namespace])) return false
...
export function installLegacySettingsImport(ctx: any, config: Record<string, any>, namespace: string, entryId: string, fields: readonly string[]): void {
  if (typeof ctx.settings.register === 'function' || typeof ctx.inject !== 'function') return
  ctx.inject(['appReady'], (readyCtx: any) => {
