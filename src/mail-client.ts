...
export function selectAttachmentPart(
  ...
  const hasSectionId = (item: EmailAttachmentMeta) => /^\d+(?:\.\d+)*$/.test(item.part)
  if (hasSectionId(meta)) return parts.find(part => part.part === meta.part)

  const used = new Set<string>()
  // Reserve exact identities even if mixed with older parsed metadata.
  for (const item of readAttachments) if (hasSectionId(item)) used.add(item.part)
  for (let position = 0; position <= index; position++) {
    const item = readAttachments[position]
    if (hasSectionId(item)) continue
    const available = parts.filter(part => !used.has(part.part))
    const sameName = (part: AttachmentPart) => part.filename === item.filename || sanitizeFilename(part.filename) === item.filename
    const tolerance = Math.max(64, Math.ceil(item.size * 0.5))
    const sameTypeAndSize = (part: AttachmentPart) => part.contentType === item.contentType && Math.abs(part.size - item.size) <= tolerance
    ...
    if (match !== undefined) used.add(match.part)
  }
  return undefined
}
