...
function projectEndpoint(
  keys: EmailSettingsValue['imap'] | undefined,
  value: EmailSettingsValue['imap'] | undefined,
  defaults: { readonly port: number; readonly secure: boolean },
  gated: boolean,
): NonNullable<EmailConfig['imap']> | undefined {
  const endpoint: NonNullable<EmailConfig['imap']> = {}
  const host = isSet(keys?.host) && isSet(value?.host) && value?.host !== '' ? value?.host : undefined
  if (host !== undefined) endpoint.host = host
  const untouched = gated && host === undefined && value?.port === defaults.port && value?.secure === defaults.secure
  if (!untouched) {
    if (isSet(keys?.port) && isSet(value?.port)) endpoint.port = value?.port
    if (isSet(keys?.secure) && isSet(value?.secure)) endpoint.secure = value?.secure
  }
...
export function toEmailConfig(value: EmailSettingsValue, user?: Partial<EmailSettingsValue> | null): EmailConfig {
  ...
  if (isSet(imapKeys) && isSet(imapValue)) {
    ...
    if (imap !== undefined) out.imap = imap
  }
  ...
  if (isSet(smtpKeys) && isSet(smtpValue)) {
    ...
    if (smtp !== undefined) out.smtp = smtp
  }
  return out
}
