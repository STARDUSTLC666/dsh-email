...
test('QQ 式乱答服务器：无关 uid 不再被当成命中（issue #15）', async () => {
  ...
  const result = await pool.search(undefined, '生日', '', 10)
...
test('不存在的关键词：乱答服务器上返回 0 条，而不是服务器声称的 40 条', async () => {
  ...
  const result = await pool.search(undefined, 'zzzznonexistentkeyword', '', 10)
...
test('只出现在正文里的关键词仍由回退扫描找到', async () => {
  ...
  const result = await pool.search(undefined, '合同', '', 10)
  assert.equal(result.count, 1)
